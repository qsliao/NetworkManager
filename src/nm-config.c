...
{
	g_strfreev (NM_CONFIG_GET_PRIVATE (config)->warnings);
}
...
#define _IS(group_v, key_v) (strcmp (group, (""group_v)) == 0 && strcmp (key, (""key_v)) == 0)
	return    _IS (NM_CONFIG_KEYFILE_GROUP_MAIN, NM_CONFIG_KEYFILE_KEY_MAIN_NO_AUTO_DEFAULT)
	       ...
	       || (g_str_has_prefix (group, NM_CONFIG_KEYFILE_GROUPPREFIX_DEVICE    ) && !strcmp (key, NM_CONFIG_KEYFILE_KEY_DEVICE_MATCH_DEVICE));
}
...
static const ConfigGroup config_groups[] = {
	...
	},
	{
		...
		.keys = (char *[]) {
			...
			NM_CONFIG_KEYFILE_KEY_DEVICE_MATCH_DEVICE,
			NM_CONFIG_KEYFILE_KEY_DEVICE_SRIOV_NUM_VFS,
			NM_CONFIG_KEYFILE_KEY_DEVICE_STOP_MATCH,
                    ...
                    GError **error)
{
	...
	*out_warnings = (char **) g_ptr_array_free (g_steal_pointer (&warnings),
	                                            warnings->len == 1);
