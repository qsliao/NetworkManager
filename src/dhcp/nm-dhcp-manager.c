              ...
              GError **error)
{
	 ...
	 *   If present, it is taken. If not present, the DHCP plugin uses a default.
	 *   For "internal" plugin, the default is just "duid". For "dhclient", we try to get
	 *   the configuration from dhclient's /etc/dhcp or fallback to whatever dhclient uses
	 *   by default. We do it this way, because for dhclient the user may configure a default
	 *   outside of NM, and we want to honor that.
	 *   Worse, dhclient could be a wapper script where the wrapper script overwrites the
	 *   client-id. We need to distinguish between: force a particular client-id and leave
	 *   it unspecified.
	 *
	 ...
	 *   For "internal" plugin @enforce_duid doesn't matter and the given client-id is always
	 *   used (simple).
	 *   For "dhclient", @enforce_duid FALSE means to first try to load the DUID from the
	 *   lease file, and only otherwise fallback to the given client-id. It's done this way,
	 *   so that existing dhclient setups don't change behavior on upgrade.
	 *
	 * This difference is cumbersome.
	 */
